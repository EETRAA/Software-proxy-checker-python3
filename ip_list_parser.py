...
def ip_parser(input_file_name,output_file_name):
  ...
  for each in input_file:
    ip=re.findall(r'(?<![\.\d])(?:\d{1,3}\.){3}\d{1,3}(?![\.\d])',str(each),re.S)#:[0-9]{1,5}#moved from ip parser re formula
    #print(ip)#debug
  ...
  for each in ip_list:
    ...
    if each != []:
      output_file.write(str(each).strip('\'[]') + "\n")#strip()_used_to_delete_redundant_characters
  #close_all_files
  output_file.close()
  input_file.close()
